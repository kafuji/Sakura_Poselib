...
from .mmd import is_mmd_tools_installed, get_mmd_root
...
def quaternion_to_mmd_degrees(q) -> tuple:
	# Quaternion to Euler
	euler = q.to_euler()
	
	# Euler to Degrees
	euler_degrees = tuple(map(lambda angle: degrees(angle), euler))
	
	return euler_degrees

# helper: Euler (degrees) to Quaternion
def mmd_rotation_to_quaternion(euler) -> Quaternion:
	# Degrees to Euler
	euler_radians = tuple(map(lambda angle: radians(angle), euler))
	
	# Euler to Quaternion
	q = Quaternion(euler_radians)
	...
	with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
		...
		for pose in poses:
			...
			for index, bone in enumerate(pose.bones):
				...
				loc = Vector( bone.location ) * 12.5
				rot = quaternion_to_mmd_degrees( Quaternion( bone.rotation ) )
				bone_data = f'PmxBoneMorph,"{pose.name}",{index},"{bone.name}",{loc.x:.5g},{loc.y:.5g},{loc.z:.5g},{rot[0]:.5g},{rot[1]:.5g},{rot[2]:.5g}\n'
				csvfile.write(bone_data)
		
		# Write comment: "This file is generated by Sakura Poselib addon for Blender."
	...
	with open(filename, 'r', newline='', encoding='utf-8') as csvfile:
		...
		for row in reader:
			...
			if row[0].startswith('PmxBoneMorph'):
				...
				rot = mmd_rotation_to_quaternion( rot )
